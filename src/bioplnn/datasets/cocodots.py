...
class CocoDots(Dataset):
    def __init__(
        ...
        self.subset = subset
        self.data = self.read_anns(
            anns_file, shuffle=self.shuffle, subset=self.subset
        )
        self.create_index()
    ...
    def create_index(self):
        ...
        if "serrelab_anns" in self.data:
            for serrelab_ann in self.data["serrelab_anns"]:
                img_to_serrelab_anns[serrelab_ann["image_id"]].append(
                    serrelab_ann
                )
    ...
    def __getitem__(self, idx):
        try:
            ...
            img = Image.open(
                os.path.join(self.img_dir, img_data["file_name"])
            ).convert("RGB")
            img = np.array(img)
            ...
            dots_h_factor = (
                self.size[0] / dots_height
            )  # factors to rescale dot coordinates
            dots_w_factor = self.size[1] / dots_width

            h_factor = (
                self.size[0] / img_data["height"]
            )  # factors to rescale image
            w_factor = self.size[1] / img_data["width"]

            # Find resized dot locations
            serrelab_ann_resize = copy.deepcopy(serrelab_ann)
            serrelab_ann_resize["cue_xy"] = [
                round(serrelab_ann_resize["cue_xy"][0] * dots_w_factor),
                round(serrelab_ann_resize["cue_xy"][1] * dots_h_factor),
            ]
            serrelab_ann_resize["fixation_xy"] = [
                round(serrelab_ann_resize["fixation_xy"][0] * dots_w_factor),
            ...
            things = [
                ...
                if self.cats[segm["category_id"]]["isthing"] == 1
            ]
            ...
            img = make_stimulus(
                ...
                segm_contours,
                thickness=thickness,
                condition=self.stimulus_condition,
            )
            ...
            dot_channel[
                serrelab_ann_resize["fixation_xy"][1],
                serrelab_ann_resize["fixation_xy"][0],
            ] = 255
            dot_channel[
                serrelab_ann_resize["cue_xy"][1],
                serrelab_ann_resize["cue_xy"][0],
            ] = 255
            ...
            print(
                "Error in getting sample with index {}, image_id {}, and serre_lab sample {}: {}".format(
                    idx,
                    img_data["id"],
                    serrelab_ann["serrelab_sample"],
                    str(e),
                )
            )
...
def make_stimulus(image, segm_contours, thickness=2, condition="WhiteOutline"):
    ...
    for contours in segm_contours:
        ...
        template_all = cv2.drawContours(
            template_all,
            [np.array(contour).astype("int32") for contour in contours],
            contourIdx=-1,
            color=(100, 100, 100),
            thickness=cv2.FILLED,
        )
